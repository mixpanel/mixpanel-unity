 ...
 */
typedef mixpanel_mbedtls_ecp_keypair mixpanel_mbedtls_ecdsa_context;
 ...
 *                  (Not thread-safe to use same context in multiple threads)
 *
 * \deprecated      Superseded by mixpanel_mbedtls_ecdsa_write_signature() in 2.0.0
 *
 * \param ctx       ECDSA context
 * \param hash      Message hash
 * \param hlen      Length of hash
...
int mixpanel_mbedtls_ecdsa_write_signature_det( mixpanel_mbedtls_ecdsa_context *ctx,
                               const unsigned char *hash, size_t hlen,
                               unsigned char *sig, size_t *slen,
                               mixpanel_mbedtls_md_type_t md_alg ) MBEDTLS_DEPRECATED;
