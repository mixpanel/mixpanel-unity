...
{
    memset( ctx, 0, sizeof( mixpanel_mbedtls_sha1_context ) );
}
...
void mixpanel_mbedtls_sha1_free( mixpanel_mbedtls_sha1_context *ctx )
{
    ...
    mixpanel_mbedtls_zeroize( ctx, sizeof( mixpanel_mbedtls_sha1_context ) );
}
...
void mixpanel_mbedtls_sha1_update( mixpanel_mbedtls_sha1_context *ctx, const unsigned char *input, size_t ilen )
{
    ...
    if( left && ilen >= fill )
    {
        ...
        mixpanel_mbedtls_sha1_process( ctx, ctx->buffer );
        input += fill;
        ilen  -= fill;
        left = 0;
    }
    ...
    while( ilen >= 64 )
    {
        mixpanel_mbedtls_sha1_process( ctx, input );
        input += 64;
        ilen  -= 64;
    }
...
void mixpanel_mbedtls_sha1_finish( mixpanel_mbedtls_sha1_context *ctx, unsigned char output[20] )
{
    ...
    mixpanel_mbedtls_sha1_update( ctx, sha1_padding, padn );
    mixpanel_mbedtls_sha1_update( ctx, msglen, 8 );
...
{
    mixpanel_mbedtls_sha1_context ctx;

    mixpanel_mbedtls_sha1_init( &ctx );
    mixpanel_mbedtls_sha1_starts( &ctx );
    mixpanel_mbedtls_sha1_update( &ctx, input, ilen );
    mixpanel_mbedtls_sha1_finish( &ctx, output );
    mixpanel_mbedtls_sha1_free( &ctx );
}
...
int mixpanel_mbedtls_sha1_self_test( int verbose )
{
    ...
    {
        if( verbose != 0 )
            mixpanel_mbedtls_printf( "  SHA-1 test #%d: ", i + 1 );
        ...
        if( i == 2 )
        {
            ...
            for( j = 0; j < 1000; j++ )
                mixpanel_mbedtls_sha1_update( &ctx, buf, buflen );
        }
        else
            mixpanel_mbedtls_sha1_update( &ctx, sha1_test_buf[i],
                               sha1_test_buflen[i] );

        mixpanel_mbedtls_sha1_finish( &ctx, sha1sum );
