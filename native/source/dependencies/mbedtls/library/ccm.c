...
{
    memset( ctx, 0, sizeof( mixpanel_mbedtls_ccm_context ) );
}
...
{
    mixpanel_mbedtls_cipher_free( &ctx->cipher_ctx );
    mixpanel_mbedtls_zeroize( ctx, sizeof( mixpanel_mbedtls_ccm_context ) );
}
...
#define UPDATE_CBC_MAC                                                      \
    for( i = 0; i < 16; i++ )                                               \
        y[i] ^= b[i];                                                       \
                                                                            \
    if( ( ret = mixpanel_mbedtls_cipher_update( &ctx->cipher_ctx, y, 16, y, &olen ) ) != 0 ) \
        return( ret );
...
#define CTR_CRYPT( dst, src, len  )                                            \
    if( ( ret = mixpanel_mbedtls_cipher_update( &ctx->cipher_ctx, ctr, 16, b, &olen ) ) != 0 )  \
        return( ret );                                                         \
                                                                               \
    for( i = 0; i < len; i++ )                                                 \
        dst[i] = src[i] ^ b[i];
 ...
 */
static int ccm_auth_crypt( mixpanel_mbedtls_ccm_context *ctx, int mode, size_t length,
...
{
    mixpanel_mbedtls_ccm_context ctx;
    unsigned char out[32];
    size_t i;
    int ret;

    mixpanel_mbedtls_ccm_init( &ctx );

    if( mixpanel_mbedtls_ccm_setkey( &ctx, MBEDTLS_CIPHER_ID_AES, key, 8 * sizeof key ) != 0 )
    {
        if( verbose != 0 )
            mixpanel_mbedtls_printf( "  CCM: setup failed" );
    ...
    {
        if( verbose != 0 )
            mixpanel_mbedtls_printf( "  CCM-AES #%u: ", (unsigned int) i + 1 );

        ret = mixpanel_mbedtls_ccm_encrypt_and_tag( &ctx, msg_len[i],
        ...
        ret = mixpanel_mbedtls_ccm_auth_decrypt( &ctx, msg_len[i],
                                iv, iv_len[i], ad, add_len[i],
                                res[i], out,
